...
from queries import (query1, query2, query3, query4, query5, query6, query7,
                      query8, query9, query10, query10_b, query11)
...
def main():
    ...
    print()
    prior_day_str = input(f'Please enter the prior day (yyyy-mm-dd): ')
  ...
  # Progress bar for remaining queries
    with tqdm(total=9, desc="Running queries", leave=False, ncols=100) as pbar:
        df_q1 = pd.read_sql_query(query1(tp, tply, prior_day), engine)
        pbar.update(1)
