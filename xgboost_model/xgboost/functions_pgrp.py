...
# Now import paths from paths.py
from paths import get_pickle_path
...
def load_dataframe_from_pickle():
    ...
    """
    # Use get_pickle_path to determine the path dynamically
...
def create_future_with_features(df):
    ...
    holidays = pd.to_datetime([
        ...
        '2024-12-26', '2024-12-31','2025-01-01', '2025-01-20', '2025-02-17', '2025-05-26',
...
def df_acts_fcts(df, future_df, year_starting=2025):
    """
...
def calculate_monthly_forecast(df):
    """
    Calculate the total forecasted estimate for the current and next month,
    formatted with commas and rounded to the nearest dollar.
    
    Parameters:
    df (pd.DataFrame): DataFrame containing forecasted data with a datetime index and an 'est' column.
    
    Returns:
    dict: A dictionary with the total estimate for the current month and the next month, formatted.
    """
    # Ensure the index is a datetime index
    if not isinstance(df.index, pd.DatetimeIndex):
        raise ValueError("The DataFrame index must be a datetime index.")

    # Get the current date and calculate start of the current and next month
    current_date = datetime.now()
    current_month_start = current_date.replace(day=1)
    next_month_start = (current_month_start + MonthBegin(1))

    # Filter for the current month and next month
    current_month_df = df[(df.index >= current_month_start) & (df.index < next_month_start)]
    next_month_df = df[(df.index >= next_month_start) & (df.index < (next_month_start + MonthBegin(1)))]

    # Calculate the sum of the 'est' column for each month
    current_month_total = current_month_df['est'].sum()
    next_month_total = next_month_df['est'].sum()

    # Format the results with commas and round to the nearest dollar
    current_month_total_formatted = f"{round(current_month_total):,}"
    next_month_total_formatted = f"{round(next_month_total):,}"

    # Return results as a dictionary with formatted values
    return {
        "current_month_total": current_month_total_formatted,
        "next_month_total": next_month_total_formatted
    }
