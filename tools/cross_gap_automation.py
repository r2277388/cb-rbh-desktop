...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Manage the scheduled weekly Cross Gap report."
    )
    parser.add_argument(
        "command",
...
def build_report_command() -> str:
    python_executable = ROOT_DIR / "venv" / "Scripts" / "python.exe"
    return f'"{python_executable}" "{process_paths.CROSS_GAP_SCRIPT}" run'


def print_status() -> int:
    exists, details = query_task_status()

    print("Weekly Cross Gap Automation")
    print(f"  Task name:      {process_paths.CROSS_GAP_TASK_NAME}")
    print(f"  Location:       {process_paths.CROSS_GAP_TASK_LOCATION}")
    print(f"  Schedule:       {process_paths.CROSS_GAP_SCHEDULE_DESCRIPTION}")
    print(f"  Script:         {process_paths.CROSS_GAP_SCRIPT}")
    print(f"  Output folder:  {process_paths.CROSS_GAP_OUTPUT_DIR}")
    print(f"  Run command:    {build_report_command()}")
...
def register_task() -> int:
    result = subprocess.run(
        [
            ...
            "WEEKLY",
            "/D",
            process_paths.CROSS_GAP_SCHEDULE_DAY,
            "/ST",
            process_paths.CROSS_GAP_SCHEDULE_TIME,
            "/TR",
