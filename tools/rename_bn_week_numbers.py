...
BN_ROOT = Path(r"G:\SALES\2026 Sales Reports\Sell-Through Reporting\Barnes & Noble")
BN_PATTERN = re.compile(
    r"^Week (?P<week>\d{1,2}) - (?P<year>\d{4}) "
    r"Rolling Barnes & Noble \((?P<date>\d{6})\)\.xlsx$",
    re.IGNORECASE,
)
...
def planned_rename(path: Path) -> RenamePlan | None:
    match = BN_PATTERN.match(path.name)
    if match is None:
        return None

    week_end = pd.to_datetime(match.group("date"), format="%m%d%y", errors="raise")
    bookscan = bookscan_week(week_end)
    new_name = BN_PATTERN.sub(
        rf"Week {bookscan.week:02d} - {bookscan.year} Rolling Barnes & Noble (\g<date>).xlsx",
        path.name,
    )
...
def collect_plans() -> list[RenamePlan]:
    if not BN_ROOT.exists():
        raise FileNotFoundError(f"B&N report folder not found: {BN_ROOT}")
    plans: list[RenamePlan] = []
    for path in BN_ROOT.rglob("*.xlsx"):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Rename B&N rolling report week numbers to BookScan weeks.")
    parser.add_argument("--apply", action="store_true", help="Actually rename files. Omit for dry run.")
    args = parser.parse_args()

    plans = collect_plans()
    if not plans:
        print("No B&N weekly report filenames need week-number changes.")
        return 0
