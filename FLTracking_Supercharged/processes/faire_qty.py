...
SQL_FILE = SQL_DIR / "faire_qty.sql"


def load_faire_qty(sql_path: Path = SQL_FILE) -> pd.DataFrame:
    query = sql_path.read_text(encoding="utf-8").strip()
    engine = get_connection()
    df = fetch_data_from_db(engine, query)

    required_columns = ["ISBN", "FaireQty"]
    missing_columns = [col for col in required_columns if col not in df.columns]
    if missing_columns:
        raise ValueError(
            "Faire qty query is missing required columns: "
...
def save_faire_qty_output(df: pd.DataFrame) -> Path:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    output_path = OUTPUT_DIR / "faire_qty.pkl"
...
def main() -> None:
    df = load_faire_qty()
    output_path = save_faire_qty_output(df)
