...
from paths import ATELIER_AMAZON_SALES_FOLDER


def load_sales_data(folder_path: Path, file_glob_pattern: str, columns: list) -> pd.DataFrame:
    files = list(folder_path.glob(file_glob_pattern))
    if not files:
        raise FileNotFoundError(f"No files found in {folder_path} with pattern {file_glob_pattern}")
    most_recent_file = max(files, key=lambda path: path.stat().st_mtime)
...
def clean_and_convert_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Ordered Revenue - Prior Period"] = (
        df["Ordered Revenue - Prior Period"].replace(r"[%,]", "", regex=True).fillna(0).astype(float) / 100
    )
    df["Ordered Revenue - Same Period Last Year"] = (
        df["Ordered Revenue - Same Period Last Year"].replace(r"[%,]", "", regex=True).fillna(0).astype(float) / 100
    )
    df["Ordered Units"] = df["Ordered Units"].replace(",", "", regex=True).fillna(0).astype(int)
    df["Ordered Units - Prior Period"] = (
        df["Ordered Units - Prior Period"].replace(r"[%,]", "", regex=True).fillna(0).astype(float) / 100
    )
    df["Ordered Units - Same Period Last Year"] = (
        df["Ordered Units - Same Period Last Year"].replace(r"[%,]", "", regex=True).fillna(0).astype(float) / 100
    )
    return df
...
def uploader_weeklysales() -> pd.DataFrame:
    folder_path = ATELIER_AMAZON_SALES_FOLDER
    file_glob_sales_weekly = "*Sales*Weekly*csv"
    cols_sales_weekly = [
        "ASIN",
        "Ordered Revenue",
