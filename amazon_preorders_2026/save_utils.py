...
def save_to_excel(df: pd.DataFrame, folder: Path) -> None:
    ...
    with pd.ExcelWriter(dated_path, engine='xlsxwriter') as dated_writer:
        ...
        df.loc[df['publisher'] == 'Galison'].to_excel(dated_writer, sheet_name='nyp_ga', index=False)
        df.loc[~df['publisher'].isin(['Chronicle', 'Galison'])].to_excel(dated_writer, sheet_name='nyp_dp', index=False)
